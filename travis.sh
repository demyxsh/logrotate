...
# Get versions
DEMYX_LOGROTATE_ALPINE_VERSION="$(docker exec "$DEMYX_REPOSITORY" cat /etc/os-release | grep VERSION_ID | cut -c 12- | sed 's/\r//g')"
DEMYX_LOGROTATE_VERSION="$(docker exec "$DEMYX_REPOSITORY" "$DEMYX_REPOSITORY" --version | head -n 1 | awk -F '[ ]' '{print $2}' | sed 's/\r//g')"
